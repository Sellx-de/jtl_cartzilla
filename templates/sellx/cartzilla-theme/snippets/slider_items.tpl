    ...
    {foreach $items as $item}
        {if $type === 'product'}
            {block name='snippets-slider-items-product'}
                <div class="tns-item" {if $tplscope !== 'box'}{if isset($Link) && $Link->getLinkType() === $smarty.const.LINKTYP_STARTSEITE || $nSeitenTyp === $smarty.const.PAGE_ARTIKELLISTE}itemprop="about"{else}itemprop="isRelatedTo"{/if} itemscope itemtype="https://schema.org/Product"{/if}>
        ...
        {elseif $type === 'freegift'}
            ...
            {block name='snippets-slider-items-freegift'}
                <div class="tns-item freegift{if $item->getStillMissingAmount() > 0} not-available-yet{/if}">
