                        ...
                        {if $hasItems}
                            <li class="link-group-item nav-item {if $hasItems}dropdown{/if}{if $active} active{/if}">
                                {block name='snippets-linkgroup-recursive-link'}
                                    <a class="footer-link dropdown-toggle" target="_self" href="{$li->getURL()}" data-toggle="collapse" data-bs-toggle="collapse"
                                       data-target="#link_box_{$li->getID()}_{$i}" data-bs-target="#link_box_{$li->getID()}_{$i}"
                    ...
                    {block name='snippets-linkgroup-recursive-mega'}
                        {block name='snippets-linkgroup-mega-recursive-main-link'}
                            {link href=$mainLink->getURL()
                                ...
                                class="d-lg-block {if $firstChild}submenu-headline submenu-headline-toplevel{/if} {$subCategory} {if $mainLink->getChildLinks()->count() > 0}footer-link dropdown-toggle{/if}"
                                aria=["expanded"=>"false"]
                        ...
                        {if $mainLink->getChildLinks()->count() > 0 && $Einstellungen.template.megamenu.show_subcategories !== 'N'}
                            {block name='snippets-linkgroup-recursive-mega-child-content'}
                                <div class="categories-recursive-dropdown dropdown-menu" data-bs-popper="static">
                                    {nav}
                                    {block name='snippets-linkgroup-recursive-mega-child-header'}
                                        <li class="nav-item d-lg-none">
                                            {link class="nav-link animate-underline animate-target d-inline fw-normal text-truncate p-0" href=$mainLink->getURL() target=$mainLink->getTarget()}
                                            ...
                                            {else}
                                                {block name='snippets-linkgroup-recursive-mega-child-link-no-child'}
                                                    {navitem href=$link->getURL()
                                                        ...
                                                        <span class="text-truncate d-block nav-link animate-underline animate-target d-inline fw-normal text-truncate p-0">
                                                            {$link->getName()}
                                                        </span>
                                                    {/navitem}
                                                {/block}
