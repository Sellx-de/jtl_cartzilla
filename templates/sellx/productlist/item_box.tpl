{block name='productlist-item-box'}
    ...
    {/if}
    <div id="{$idPrefix|default:''}result-wrapper_buy_form_{$Artikel->kArtikel}" data-wrapper="true" data-bs-wrapper="true"
        ...
        data=["toggle" => "basket-add","bs-toggle" => "basket-add"]}
        {input type="hidden" name="a" value="{if !empty({$Artikel->kVariKindArtikel})}{$Artikel->kVariKindArtikel}{else}{$Artikel->kArtikel}{/if}"}
        <div class="productbox-inner">
            {row}
                {col cols=12}
                    <div class="productbox-image" data-target="#variations-collapse-{$Artikel->kArtikel}" data-bs-target="#variations-collapse-{$Artikel->kArtikel}">
