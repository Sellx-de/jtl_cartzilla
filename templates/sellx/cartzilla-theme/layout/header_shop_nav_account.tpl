{block name='layout-header-shop-nav-account'}
    {navitemdropdown tag="li"
        ...
        no-caret=true
        right=true
        text="{if JTL\Session\Frontend::getCustomer()->getID() === 0}<span class=\"ci-user animate-target\"></span>{else}<span class=\"ci-user animate-target\"></span>{/if}"
        class="btn btn-icon btn-lg fs-lg btn-outline-secondary border-0 rounded-circle animate-shake d-none d-md-inline-flex account-icon-dropdown popper-static-dropdown"}
