{block name='checkout-inc-shipping-address'}
    ...
    {block name='checkout-inc-shipping-address-checkbox-equals'}
        <div class="form-group checkbox control-toggle">
            ...
            data=["toggle"=>"collapse","bs-toggle"=>"collapse", "target"=>"#select-shipping-address","bs-target"=>"#select-shipping-address"]
                ...
                {if JTL\Session\Frontend::getCustomer()->getID() > 0 && isset($Lieferadressen) && $Lieferadressen|count > 0}
                                ...
                                <tr>
                                    <td>
                                        <label class="btn-block no-caret text-wrap" for="delivery{$adresse->kLieferadresse}"
                                               data-toggle="collapse" data-bs-toggle="collapse" data-target="#register_shipping_address.show" data-bs-target="#register_shipping_address.show">
                                    ...
                                    <td>
                                        ...
                                        {button variant="link" class="btn-show-more"
                                            data=["toggle"=> "collapse","bs-toggle"=> "collapse", "target"=>"#deliveryAdditional{$adresse->kLieferadresse}","bs-target"=>"#deliveryAdditional{$adresse->kLieferadresse}"]}
                                            {lang key='showMore'}
                                        {/button}
                                    </td>
                    ...
                    {block name='checkout-inc-shipping-address-new-address-wrapper'}
                        ...
                        {block name='checkout-inc-shipping-address-fieldset-new-address'}
                            <label class="btn-block" for="delivery_new" data-toggle="collapse" data-bs-toggle="collapse" data-target="#register_shipping_address:not(.show)" data-bs-target="#register_shipping_address:not(.show)">
