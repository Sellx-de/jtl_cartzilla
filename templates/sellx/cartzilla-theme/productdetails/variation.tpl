{block name='productdetails-variation'}
    {if isset($Artikel->Variationen) && $Artikel->Variationen|@count > 0 && !$showMatrix}
        ...
        {block name='productdetails-variation-spinner'}
            <div class="updatingStockInfo text-center d-none">
                <i class="ci-loading fs-xl" title="{lang key='updatingStockInformation' section='productDetails'}"></i>
            </div>
        {/block}
            ...
            <div class="variations {if $simple}simple{else}switch{/if}-variations">
                <div class="mb-3">
                    {foreach name=Variationen from=$Artikel->$VariationsSource key=i item=Variation}
                        ...
                        <div class="variation-wrapper mb-3 {if $Variation->cTyp === 'IMGSWATCHES'}js-slider-wrapper {if !$showSwatchSlider}js-slider-disabled{/if}{/if}">
                    {strip}
                        {block name='productdetails-variation-name-outer'}
                        <div class="d-flex align-items-center mb-2 js-btn-slider-wrapper">
                            {block name='productdetails-variation-name'}
                                <label class="form-label fw-medium mb-0 me-2">
                            ...
                            {if $Variation->cTyp === 'IMGSWATCHES'}
                                    <span class="swatches-selected text-accent" data-id="{$Variation->kEigenschaft}" data-bs-id="{$Variation->kEigenschaft}">
                                ...
                                {if $showSwatchSlider}
                                    <div class="js-btn-slider-btns ms-auto">
                                        <button type="button" class="btn btn-sm btn-icon btn-outline-secondary js-btn-slider-sb" disabled>
                                            <i class="ci-arrow-left"></i>
                                        </button>
                                        <button type="button" class="btn btn-sm btn-icon btn-outline-secondary js-btn-slider-sf">
                                        ...
                                        {else}
                                            {block name='productdetails-variation-select-inner'}
                                                ...
                                                <option value="{$Variationswert->kEigenschaftWert}" class="variation"
                                                    ...
                                                    {if $Variationswert->notExists} <span class='badge text-bg-danger badge-not-available'>{lang key='notAvailableInSelection'}</span>
                                                    {elseif !$Variationswert->inStock}<span class='badge text-bg-danger badge-not-available'>{lang key='ampelRot'}</span>{/if}</span>"
                                                        data-type="option" data-bs-type="option"
                                                        data-original="{$Variationswert->cName}" data-bs-original="{$Variationswert->cName}"
                                                        data-key="{$Variationswert->kEigenschaft}" data-bs-key="{$Variationswert->kEigenschaft}"
                                                        data-value="{$Variationswert->kEigenschaftWert}" data-bs-value="{$Variationswert->kEigenschaftWert}"
                                                        ...
                                                        {if isset($Variationswert->oVariationsKombi)}
                                                            data-ref="{$Variationswert->oVariationsKombi->kArtikel}" data-bs-ref="{$Variationswert->oVariationsKombi->kArtikel}"
                                                        {/if}
                                            ...
                                            {block name='productdetails-variation-radio-inner'}
                                                <div class="custom-control custom-radio">
                                                    ...
                                                    <label class="variation custom-control-label d-flex" for="{if isset($smallView) && $smallView}a-{$Artikel->kArtikel}{/if}vt{$Variationswert->kEigenschaftWert}"
                                                           data-type="radio" data-bs-type="radio"
                                                           data-original="{$Variationswert->cName}" data-bs-original="{$Variationswert->cName}"
                                                           data-key="{$Variationswert->kEigenschaft}" data-bs-key="{$Variationswert->kEigenschaft}"
                                                           data-value="{$Variationswert->kEigenschaftWert}" data-bs-value="{$Variationswert->kEigenschaftWert}"
                                                           {if !empty($Variationswert->getImage(\JTL\Media\Image::SIZE_XS))}
                                                           ...
                                                           {if !$Variationswert->inStock}
                                                                data-stock="out-of-stock" data-bs-stock="out-of-stock"
                                                           {/if}
                                                           {if isset($Variationswert->oVariationsKombi)}
                                                                data-ref="{$Variationswert->oVariationsKombi->kArtikel}" data-bs-ref="{$Variationswert->oVariationsKombi->kArtikel}"
                                                           {/if}>
                                            ...
                                            {else}
                                                ...
                                                {col class='col-auto js-slider-item'}
                                                    <label class="variation swatches {if $hasImage}swatches-image{else}swatches-text{/if} {if $bSelected}active{/if} {if $Variationswert->notExists}swatches-not-in-stock not-available{elseif !$Variationswert->inStock}swatches-sold-out not-available{/if}"
                                                            data-type="swatch" data-bs-type="swatch"
                                                            data-original="{$Variationswert->cName}" data-bs-original="{$Variationswert->cName}"
                                                            data-key="{$Variationswert->kEigenschaft}" data-bs-key="{$Variationswert->kEigenschaft}"
                                                            data-value="{$Variationswert->kEigenschaftWert}" data-bs-value="{$Variationswert->kEigenschaftWert}"
                                                            ...
                                                            {if $Variationswert->notExists}
                                                                ...
                                                                {if !$showSwatchSlider}data-toggle="tooltip" data-bs-toggle="tooltip"{/if}
                                                            {elseif $Variationswert->inStock}
                                                                data-title="{$Variationswert->cName}" data-bs-title="{$Variationswert->cName}"
                                                            {else}
