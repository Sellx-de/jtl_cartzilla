                            ...
                            {block name='comparelist-index-filter-buttons'}
                                <div class="d-flex flex-wrap justify-content-between align-items-center">
                                    <div class="d-flex align-items-center mb-3 mb-sm-0">
                                        <button class="btn btn-outline-primary me-2" type="button" data-bs-toggle="collapse" data-bs-target="#collapse-checkboxes" aria-expanded="false" aria-controls="collapse-checkboxes">
                                            <i class="ci-filter me-2"></i>{lang key='filter'}
                                        </button>
                                        <button class="btn btn-outline-primary me-2" type="button" id="check-all">
                                            <i class="ci-eye me-2"></i>{lang key='showAll'}
                                        </button>
                                        <button class="btn btn-outline-primary" type="button" id="check-none">
                                            <i class="ci-eye-off me-2"></i>{lang key='showNone'}
                                        </button>
                                    </div>
                        {/block}
                                                ...
                                                {col cols=6 md=4 lg=3 xl=2 class="comparelist-checkbox-wrapper"}
                                                    {checkbox checked=true data=['id' => "attr-{$characteristic->getName($languageID)|default:$characteristic->getName($fallback)}"] class='comparelist-checkbox'}
                                                        <div class="text-truncate">
                                                            {$characteristic->getName()|default:$characteristic->getName($fallback)|escape:'html'}
                                                        </div>
                                                    {/checkbox}
                                                {/col}
                                            {/foreach}
                        ...
                        <table class="table table-bordered table-hover">
                                                    ...
                                                    {block name='comparelist-index-products-header-delete'}
                                                        <div class="delete-link-times">
                                                            {link href=$oArtikel->cURLDEL
                                                                ...
                                                                data=["toggle"=>"tooltip","bs-toggle"=>"tooltip"]}
                                                                <i class="fas fa-times"></i>
                                                            {/link}
                                                        </div>
                                                    {/block}
                                                    {block name='comparelist-index-products-header-image'}
                                                        {link href=$oArtikel->cURLFull class="d-block"}
                                                            {include file='snippets/image.tpl' item=$oArtikel
                                                            srcSize='md' square=false class='comparelist-item-image'
                                                            sizes='(max-width: 991.98px) 300px, (max-width: 1299.98px) 365px, (min-width: 1300px) 510px, 250px'}
                                                        {/link}
                                                    {/block}
                                                </div>
                            ...
                            {foreach $prioRows as $row}
                                {if $row['key'] !== 'Merkmale' && $row['key'] !== 'Variationen'}
                                    ...
                                    {/block}
                                    {block name='comparelist-index-products-row-content'}
                                        {foreach $oVergleichsliste->oArtikel_arr as $oArtikel}
                                                    ...
                                                    {elseif $row['key'] === 'cBeschreibung' || $row['key'] === 'cKurzBeschreibung'}
                                                        {block name='comparelist-index-products-row-description'}
                                                            ...
                                                            {else}
                                                                ...
                                                                button-label-show="{lang key='more'}" button-label-hide="{lang key='less'}"
                                                                clip-text="{substr($oArtikel->$row['key']|strip_tags, 0, $descriptionLength)}{if strlen($oArtikel->$row['key']|strip_tags) > $descriptionLength}...{/if}"}
                                                                    {$oArtikel->$row['key']}
                                                                {/collapse}
                                                            {/if}
                                                        {/block}
                                ...
                                {elseif $row['key'] === 'Merkmale'}
                                    {block name='comparelist-index-characteristics'}
                                        {foreach $oMerkmale_arr as $characteristic}
                                            ...
                                            <tr class="comparelist-row" data-id="row-attr-{$name}" data-bs-id="row-attr-{$name}">
                                                <td class="comparelist-label">
                                                    {$name|truncate:20}
                                                </td>
                                                {foreach $oVergleichsliste->oArtikel_arr as $oArtikel}
                                ...
                                {elseif $row['key'] === 'Variationen'}
                                    {block name='comparelist-index-variations'}
                                        {foreach $oVariationen_arr as $oVariationen}
                                            <tr class="comparelist-row" data-id="row-vari-{$oVariationen->cName}" data-bs-id="row-vari-{$oVariationen->cName}">
