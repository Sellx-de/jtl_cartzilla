<div data-tnt-template=boxes/box_filter_characteristics.tpl></div>{block name='boxes-box-filter-characteristics'}
        ...
        {foreach $oBox->getItems() as $characteristic}
            <div id="sidebox{$oBox->getID()}-{$characteristic->getID()}" class="box box-filter-characteristics d-none d-lg-block">
                {button
                    ...
                    block=true
                    data=["toggle"=> "collapse","bs-toggle"=> "collapse", "target"=>"#cllps-box{$oBox->getID()}-{$characteristic->getID()}","bs-target"=>"#cllps-box{$oBox->getID()}-{$characteristic->getID()}"]
                }
