                                    ...
                                    <tr>
                                        <td>
                                            ...
                                            {button variant="link" class="btn-show-more"
                                                data=["toggle"=> "collapse","bs-toggle"=> "collapse", "target"=>"#deliveryAdditional{$address->kLieferadresse}","bs-target"=>"#deliveryAdditional{$address->kLieferadresse}"]}
                                                {lang  key='showMore'}
                                            {/button}
                                        </td>
                                        <td class="text-right">
                                            {buttongroup}
                                                {if $Einstellungen.kaufabwicklung.bestellvorgang_kaufabwicklungsmethode == 'N' && $address->nIstStandardLieferadresse !== 1}
                                                    <button type="button" class="btn btn-outline-secondary btn-sm" data-toggle="tooltip" data-bs-toggle="tooltip" data-placement="top" data-bs-placement="top" title="{lang key='useAsDefaultShippingAddress' section='account data'}" onclick="location.href='{get_static_route id='jtl.php' params=['editLieferadresse' => 1, 'setAddressAsDefault' => {$address->kLieferadresse}]}'">
                                                        {lang key='setAsStandard' section='account data'}
                                                    </button>
                                                {/if}

                                                <button type="button" class="btn btn-secondary btn-sm" data-toggle="tooltip" data-bs-toggle="tooltip" data-placement="top" data-bs-placement="top" title="{lang key='editAddress' section='account data'}" onclick="location.href='{get_static_route id='jtl.php' params=['editLieferadresse' => 1, 'editAddress' => {$address->kLieferadresse}]}'">
                                                    <span class="fas fa-pencil-alt"></span>
                                                </button>

                                                <button type="button" class="btn btn-danger btn-sm delete-popup-modal" data-lieferadresse="{$address->kLieferadresse}" data-bs-lieferadresse="{$address->kLieferadresse}" data-toggle="tooltip" data-bs-toggle="tooltip" data-placement="top" data-bs-placement="top" title="{lang key='deleteAddress' section='account data'}">
